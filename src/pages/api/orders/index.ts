...
export default async function handler(
  ...
  try {
    ...
    if (paymentMethod.type === 'CREDIT_CARD' && paymentMethod.stripePaymentMethodId) {
      ...
      try {
        console.log(`決済処理開始 - PaymentMethodID: ${paymentMethod.stripePaymentMethodId}`);
        ...
        if (!stripeCustomerId) {
          return res.status(400).json({ 
            success: false, 
            error: 'ユーザー情報が正しく設定されていないため、決済を処理できません。'
          });
        }
        ...
        console.log(`既存の支払い方法数: ${customerPaymentMethods.data.length}`);
        ...
        if (!existingPaymentMethod) {
          console.log('支払い方法がカスタマーに関連付けられていません。再アタッチを試みます。');
          
          try {
            // 先に既存の支払い方法をデタッチする試み
            try {
              console.log(`支払い方法をデタッチします: ${paymentMethodIdToUse}`);
              await stripe.paymentMethods.detach(paymentMethodIdToUse);
              console.log('デタッチ成功');
            } catch (detachError) {
              console.log('デタッチ中にエラーが発生しました（無視して続行）:', 
            ...
            return res.status(400).json({ 
              success: false, 
              error: '支払い方法に問題があります。新しいカード情報を入力してください。',
              clientSecret: setupIntent.client_secret,
              requiresAction: true
            });
          }
        } else {
          console.log(`支払い方法がカスタマーに既に存在します: ${existingPaymentMethod.id}`);
        }
        ...
        console.log(`支払い処理実行 - 金額: ${amountInYen}円, 顧客ID: ${stripeCustomerId}`);
        ...
        console.log(`PaymentIntent作成成功 - ID: ${paymentIntent.id}, ステータス: ${paymentIntent.status}`);
        ...
        if (paymentIntent.status === 'succeeded') {
          ...
          try {
            ...
            order = await prisma.$transaction(async (tx) => {
              ...
              const inventoryStatus = await validateInventoryInTransaction(tx, items);
        ...
        // Stripeエラーの詳細な処理
        let errorMessage = 'クレジットカード決済に失敗しました。カード情報をご確認ください。';
        
        if (stripeError instanceof Stripe.errors.StripeError) {
          // 特定のエラーコードに対する詳細なメッセージ
          if (stripeError.code === 'payment_intent_authentication_failure') {
            errorMessage = 'カード認証に失敗しました。別のカードをお試しいただくか、カード発行会社にお問い合わせください。';
          } else if (stripeError.code === 'card_declined') {
            errorMessage = 'カードが拒否されました。別のカードをお試しください。';
          } else if (stripeError.code === 'expired_card') {
            errorMessage = 'カードの有効期限が切れています。別のカードをお試しください。';
          } else if (stripeError.code === 'payment_method_unattached_customer') {
            errorMessage = '支払い方法が正しく設定されていません。別の支払い方法をお試しください。';
          }
        }
      ...
      try {
        ...
        return res.status(400).json({
          success: false,
          error: txError instanceof Error ? txError.message : '注文処理中にエラーが発生しました'
        });
      }
...
async function checkInventory(items) {
  ...
  for (const item of items) {
    console.log(`在庫確認処理 - 商品ID: ${item.itemId}, サイズ: ${item.size}, 数量: ${item.quantity}`);
    ...
    if (!product) {
      const error = new Error(`商品ID ${item.itemId} が見つかりません`);
    ...
    const productInfo = {
      ...
      availableSizes: product.itemSizes.map(s => ({ size: s.size, inventory: s.inventory }))
    };
    
    console.log(`商品情報:`, JSON.stringify(productInfo));
    results.push(productInfo);

    // サイズ指定がある場合はサイズごとの在庫をチェック
    if (item.size) {
      const sizeStock = product.itemSizes.find(
        s => s.size === item.size
      );

      if (!sizeStock) {
        const error = new Error(`商品ID ${item.itemId} のサイズ ${item.size} が見つかりません`);
      ...
      if (sizeStock.inventory < item.quantity) {
        const error = new Error(`商品「${product.name}」のサイズ ${item.size} の在庫が不足しています。現在の在庫: ${sizeStock.inventory}`);
        (error as any).details = { 
          itemId: item.itemId, 
          itemName: product.name,
          size: item.size, 
      ...
      // サイズ管理している商品なのにサイズ指定がない
      const error = new Error(`商品「${product.name}」にはサイズの指定が必要です`);
      (error as any).details = { 
        itemId: item.itemId, 
        itemName: product.name,
        hasSizes: true, 
      ...
      // サイズ指定がない場合は商品全体の在庫をチェック
      if (product.inventory < item.quantity) {
        const error = new Error(`商品「${product.name}」の在庫が不足しています。現在の在庫: ${product.inventory}`);
        (error as any).details = { 
          itemId: item.itemId, 
          itemName: product.name,
          requestedQuantity: item.quantity, 
...
async function validateInventoryInTransaction(tx, items) {
  ...
  for (const item of items) {
    ...
    if (!product) {
      throw new Error(`商品ID ${item.itemId} が見つかりません`);
    }
    ...
    if (item.size) {
      ...
      if (!sizeStock) {
        throw new Error(`商品ID ${item.itemId} のサイズ ${item.size} が見つかりません`);
      }

      // トランザクション内での厳密なチェック - 他の同時購入がないか確認するため
      if (sizeStock.inventory < item.quantity) {
        throw new Error(`商品「${product.name}」のサイズ ${item.size} の在庫が不足しています。現在の在庫: ${sizeStock.inventory}`);
      }
      ...
      // サイズ指定がない場合は商品全体の在庫をチェック
      if (product.inventory < item.quantity) {
        throw new Error(`商品「${product.name}」の在庫が不足しています。現在の在庫: ${product.inventory}`);
      }
...
async function decreaseInventory(tx, items) {
  for (const item of items) {
    ...
    if (item.size) {
      ...
      console.log(`サイズ在庫を更新: 商品=${product.name}, サイズ=${item.size}, 更新前=${sizeStock.inventory}, 更新後=${sizeStock.inventory - item.quantity}`);
    }

    // 商品全体の在庫も減らす（サイズがあってもなくても）
    if (product.inventory < item.quantity) {
      throw new Error(`商品「${product.name}」の在庫が不足しています。現在の在庫: ${product.inventory}`);
    }

    // 商品全体の在庫を減らす - トランザクション内での更新
    await tx.item.update({
      where: { id: item.itemId },
      data: { inventory: product.inventory - item.quantity }
    });
    
    console.log(`商品在庫を更新: 商品=${product.name}, 更新前=${product.inventory}, 更新後=${product.inventory - item.quantity}`);
  }
}

// 注文データを作成する関数（トランザクション版）
async function createOrderWithTransaction(tx, userId, address, items, total, paymentReferenceId) {
